...
namespace HerdRider
{
    public class Program
    {
        ...
        public static void InitializeHerdRider(VirtualCanvas canvas)
        {
            canvas.SetOrientation(Orientation.Portrait);
            canvas.DrawFill(ColorBackground);
            canvas.DrawString(35, 10, (ushort)BasicColor.Black, DejaVuSansBold9.ID, "Herd Rider by Larouex");

            var fontInfo = new DejaVuSans9().GetFontInfo();

            var button = new ButtonWidget(10, 30, 220, 50, fontInfo, "Configure WiFi");
            button.FillColorClicked = ColorHelpers.GetRGB24toRGB565(255, 255, 255);
            button.FontColorClicked = ColorHelpers.GetRGB24toRGB565(0, 255, 0);
            button.FillColor = ColorHelpers.GetRGB24toRGB565(0, 0, 255);
        ...
        public static void TouchEventHandler(object sender, TouchEvent touchEvent) {
